...
import {doCloseDetailsModal} from '../../../actions/master/device';
...
class DetailModal extends Component {
  ...
  renderDetail() {
        return (<DeviceTabPaneAdd initialValues={this.props.selectedData} action="edit"  initializer={() => this.props.initializer()}/>);
  }
  render() {
    ...
    return(
      ...
      <Modal show={this.props.showDetailsModal} onHide={()=>this.props.doCloseDetailsModal()} keyboard={false}>
            <Modal.Header closeButton>
              ...
              <div>
              <h4 className="modal-title" id="instantLabel">Edit Device</h4>
              {
...
function mapStateToProps(state) {
  //console.log(state);
  return {
      showDetailsModal: state.device.showDetailsModal,
      selectedData: state.device.selectedData,
      popover: 'popover',
      tooltip: 'tooltip'
