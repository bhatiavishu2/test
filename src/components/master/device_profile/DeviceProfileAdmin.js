...
import { initiateState } from '../../../actions/master/device_profile';
import { changeTab,search } from '../../../actions/master/device_profile';

class DeviceProfileAdmin extends Component {
  constructor(props) {
    super(props);
    let timer = null;
  }
  ...
  initDataTable =function() {
    $('#device_profileList').DataTable({ destroy: true,"order": []});
  }
  ...
  render(){
      return (
        <div>
        		<AdminHeader title="DeviceProfiles"/>
              ...
              <ul className="nav nav-tabs" role="tablist">
                ...
                <DeviceProfileTabPaneAdd  className={this.props.activeTabId==2?"active tab-pane":"tab-pane"} action="add" initializer={() => this.initialiseList()}/>
        			</div>
        		</div>
...
function mapStateToProps(state) {
  return {
      ...
      data: state.device_profile.data,
      activeTabId:state.device_profile.activeTabId
  };
}
function mapDispatchToProps(dispatch) {
  return bindActionCreators({initiateState:initiateState, changeTab:changeTab, search:search},dispatch);
}
export default connect(mapStateToProps, mapDispatchToProps)(DeviceProfileAdmin);
